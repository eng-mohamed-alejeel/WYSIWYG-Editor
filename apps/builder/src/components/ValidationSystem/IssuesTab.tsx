...
export const IssuesTab: React.FC<IssuesTabProps> = ({ issues, isScanning, onFixIssue, onFixAll, onRescan }) => (
  <div className="space-y-4">
    {isScanning ? (
      ...
      <EmptyState icon="check-circle" title="No issues found" description="Your project looks great!" />
    ) : (
      <>
        <div className="flex gap-2 mb-4">
          <Button variant="primary" size="small" onClick={onFixAll} disabled={!issues.some(i => i.fix)}>
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex items-start gap-2 flex-1">
                  <Icon
                    name={issue.type === 'error' ? 'alert-circle' : issue.type === 'warning' ? 'alert-triangle' : 'info'}
                    size="small"
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <Badge variant={issue.type === 'error' ? 'danger' : 'secondary'} size="sm">{issue.componentType}</Badge>
